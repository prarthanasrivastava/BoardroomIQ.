...
BoardroomIQ works without an OpenAI key. If no key is configured, it falls back to the deterministic CEO summary generated by the local agent pipeline.

To enable the AI explanation layer, create a `.env` file from `.env.example`:
...
BOARDROOMIQ_LLM_MODEL=gpt-4o-mini
```

In the React UI, turn on **Use OpenAI explanation layer** before running the board meeting.
